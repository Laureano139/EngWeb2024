...
for ficheiro in os.listdir(ruas_dir):
    ...
    lista_ruas.append((nome.text, numero.text))
    
lista_ordenada = sorted(lista_ruas, key=lambda x: x[0].strip())

for nome, numero in lista_ordenada:
    ruaHTML = open(f'html/{nome}.html', 'w', encoding="utf-8")
    html += "<ul class='w3-ul w3-border w3-hoverable'>"
    html += f'<li><a href="html/{nome}.html">{nome}</a></li>'
    html += "</ul>"
    ...
    for fig in root.findall('./corpo/figura'):
        ...
        imagens.append((imagem_path, legenda))
        
    for imagem_path, legenda in imagens:
        numero_no_arquivo = re.search(r'\d+', imagem_path).group()
        numero_no_arquivo = int(numero_no_arquivo.lstrip('0'))
        if(numero_no_arquivo == int(numero)):
